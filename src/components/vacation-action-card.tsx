...
interface VacationActionCardProps {
  action: (content: Vacation) => void;
  actionType: 'edit' | 'create';
}

export function NewVacationCard({
  action,
  actionType,
}: VacationActionCardProps) {
  ...
  const onSubmit = (values: VacationFormValues) => {
    ...
    try {
      ...
      toast.error(
        actionType === 'edit'
          ? 'Failed to edit vacation. Please try again.'
          : 'Failed to create new plan. Please try again.'
      );
    } finally {
      setIsSaving(false);
    }
